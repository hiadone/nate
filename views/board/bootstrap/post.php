...
<div class="board">
	...
	<?php if (element('link_count', $view) > 0 OR element('file_download_count', $view) > 0) { ?>
		<ul class="list-group">
			...
			?>
				<li class="list-group-item"><i class="fa fa-link"></i> <a href="<?php echo element('media_click', $value); ?>" target="_blank"><?php echo html_escape(element('pln_url', $value)); ?></a><span class="badge"><?php echo number_format(element('pln_hit', $value)); ?></span>
					<?php if (element('show_url_qrcode', element('board', $view))) { ?>
	...
	//]]>
		...
		<ul class="list-group">
			...
			<li class="list-group-item">
				...
				<div class="list-group-item-text" style="min-height:20px;"><i class="fa fa-link"></i> <a href="<?php echo element('media_url', $view); ?>" target="_blank"><?php echo html_escape(element('media_url', $view)); ?></a>			<a href="<?php echo base_url('stat/view_log/'.element('brd_key', element('board',$view)));?>?post_id_[]=<?php echo element('post_id', element('post', $view)); ?>" class="btn btn-success btn-xs ml20">통계보러가기</a>
				</div>
			</li>
		</ul>
	...
	<?php
	if ( ! element('post_hide_comment', element('post', $view)) && false) {
	?>
