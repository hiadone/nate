<div class="box">
	<div class="box-table">
		...
		?>
			...
			<div class="box-table-header">
				<h4><a data-toggle="collapse" href="#cmalltab8" aria-expanded="true" aria-controls="cmalltab8">채널</a></h4>
				<a data-toggle="collapse" href="#cmalltab8" aria-expanded="true" aria-controls="cmalltab8"><i class="fa fa-chevron-up pull-right"></i></a>
			</div>
			<div class="collapse" id="cmalltab8">
				<div class="form-group">
					<label class="col-sm-2 control-label">채널</label>
					<div class="col-sm-10">
						<?php
						$open = false;
						$channel = element('all_channel', element('data', $view));
						$item_channel = element('channel', element('data', $view));
						if (element(0, $channel)) {
							$i = 0;
							foreach (element(0, $channel) as $key => $val) {
								$display = (is_array($item_channel) && in_array(element('cch_id', $val), $item_channel)) ? "block" : 'none';
								if ($i%3== 0) {
									echo '<div>';
									$open = true;
								}
								echo '<div class="checkbox-inline" style="vertical-align:top;">';
								$cat_checked = (is_array($item_channel) && in_array(element('cch_id', $val), $item_channel)) ? 'checked="checked"' : '';
								echo '<label for="cch_id_' . element('cch_id', $val) . '"><input type="checkbox" name="cmall_channel[]" value="' . element('cch_id', $val) . '" ' . $cat_checked . ' id="cch_id_' . element('cch_id', $val) . '" onclick="display_cmall_channel(this.checked,\'catwrap_' . element('cch_id', $val) . '\');" />' . element('cch_value', $val) . '</label> ';
								echo get_subcat($channel, $item_channel, element('cch_id', $val), $display);
								echo '</div>';
								if ($i%3== 2) {
									echo '</div>';
									$open = false;
						...
						<script type="text/javascript">
						//<![CDATA[
						function display_cmall_channel(check, idname) {
							if (check === true) {
								$('#' + idname).show();
							} else {
								$('#' + idname).hide();
			...
			<div class="collapse in" id="cmalltab3">
				<div class="form-group">
					...
					<div class="col-sm-10 form-inline">
						<input type="text" class="form-control" name="cit_price" value="<?php echo set_value('cit_price', element('cit_price', element('data', $view))); ?>" /> 
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label">상품쇼핌 URL</label>
					<div class="col-sm-10 ">
			...
			<div class="box-table-header">
				<h4><a data-toggle="collapse" href="#cmalltab7" aria-expanded="true" aria-controls="cmalltab7">상/하단 내용</a></h4>
