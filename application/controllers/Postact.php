...
class Postact extends CB_Controller
{
	...
	public function import_write($pid=0)
	{
		...
		$this->load->model(array('Cmall_item_model','Cmall_item_meta_model','Cmall_category_model','Cmall_category_rel_model','Cmall_channel_model','Cmall_channel_rel_model'));

		$primary_key = $this->Cmall_item_model->primary_key;
		...
		$getdata = array();
		if ($pid) {
			$getdata = $this->Cmall_item_model->get_one($pid);
			...
			$cha = $this->Cmall_channel_model->get_channel(element('cit_id', $getdata));
			if ($cha) {
				foreach ($cha as $ck => $cv) {
					$getdata['channel'][] = $cv['cch_id'];
				}
			}
		} else {
			// 기본값 설정
		...
		if ($form_validation === false OR $file_error !== '') {
			...
			$array = array(
			            'programName', 'actorName', 'roleName', 'productBrand', 'productModel',
			            'minPrice', 'maxPrice'
			        );
			...
			if($this->input->post('pplCategory')){
			    $cmall_category_a = element('cca_id',$this->Cmall_category_model->get_one('','',array('cca_num' => substr($this->input->post('pplCategory'),0,3))));

			    $cmall_category_b = element('cca_id',$this->Cmall_category_model->get_one('','',array('cca_parent' => $cmall_category_a,'cca_num' => substr($this->input->post('pplCategory'),3))));
			    $cmall_category = array($cmall_category_a,$cmall_category_b);
			    
			}

			if($this->input->post('orgChannelID')){
			    $cmall_channel = array(element('cch_id',$this->Cmall_channel_model->get_one('','',array('cch_num' => $this->input->post('orgChannelID')))));
			...
			if ($pid) {
				$this->Cmall_item_model->update($pid, $updatedata);
				...
				if(!empty($cmall_channel))
				    $this->Cmall_channel_rel_model->save_channel($pid, $cmall_channel);

				

				echo $pid.' 정상적으로 수정되었습니다';
				...
				$pid = $this->Cmall_item_model->insert($updatedata);
	...
	public function media_click($link_id = 0)
    {
