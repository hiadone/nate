...
class Cmallcategory extends CB_Controller
{
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_cmall_cmallcategory_import';
		$this->load->event($eventname);
		...
		$url = 'http://open.api.tingle.kr/category?X-OAPI-KEY=cd5a1d6d54bb0376fac5be8cf90090c5';
		$data = array();
		...
		foreach($obj as $value){
			...
			$cca_num_cnt=$this->Cmall_category_model->count_by(array('cca_num'=> element('pplCategory',$value)));
			if($cca_num_cnt) continue;

			if(element('ca_depth',$value) === '1'){
				echo "aaa";
				$insertdata = array(
					'cca_value' => element('pplCategoryName',$value),
					'cca_parent' => 0,
					'cca_order' => 0,
					'cca_num' => element('pplCategory',$value),
				);
				$this->Cmall_category_model->insert($insertdata);

			} elseif(element('ca_depth',$value) === '2'){ 
				
				$cca_num_info = $this->Cmall_category_model->get_one('','',array('cca_num'=> substr(element('pplCategory',$value),0,3)));
				
				$insertdata = array(
					'cca_value' => element('pplCategoryName',$value),
					'cca_parent' => element('cca_id',$cca_num_info),
					'cca_order' => 0,
					'cca_num' => substr(element('pplCategory',$value),3),
				);
				$this->Cmall_category_model->insert($insertdata);
