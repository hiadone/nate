...
class Board extends CI_Controller
{
	 ...
	 * 최근게시물을 가져옵니다
	 */
	public function latest_graph($config)
	{
		$view = array();
		$view['view'] = array();
		...
		$view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
		$html = $this->CI->load->view('latest/' . $skin . '/latest_graph', $view, true);
	...
	public function media_iframe($config)
	{	
        ...
        $skin = element('brd_key', $config)."/".element('post_id', $config);

        if ($cache_minute> 0) {
			$cache_post_id = is_array($post_id) ? implode('-', $post_id) : $post_id;
			$cache_brd_key = is_array($brd_key) ? implode('-', $brd_key) : $brd_key;
			
			$cachename = 'media/iframe-s-' . $brd_key."-".$post_id . '-i-' . $cache_post_id . '-k-' . $cache_brd_key . '-l-k-l-t-g-w-' . $image_width . '-h-' . $image_height . '-p';
			$html = $this->CI->cache->get($cachename);
			if ($html) {
				return $html;
			}
        ...
        $this->CI->load->model( array('Post_model', 'Post_link_model', 'Cmall_item_model','Post_extra_vars_model'));
        ...
        $post = $this->CI->Post_model->get_one('','',$where);
        ...
        $post['extravars'] = $this->CI->Post_extra_vars_model->get_all_meta(element('post_id', $post));
        ...
        if (element('post_link_count', $post)) {
            ...
            $view['view']['link'] = $link = $this->CI->Post_link_model
            ...
            if ($link && is_array($link)) {
                foreach ($link as $key => $value) {
                    ...
                    $view['view']['link'][$key]['item']  = $item = $this->CI->Cmall_item_model
                        ->get_one('', '', $itemwhere);
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'media/' . $skin);
		$html = $this->CI->load->view('media/' . $skin . '/iframe', $view, true);

		if ($cache_minute> 0) {			
			check_cache_dir('media');
			$this->CI->cache->save($cachename, $html, $cache_minute);
		}
	...
	public function media_script($config)
	{
		$post_id = element('post_id', $config);
		$brd_key = element('brd_key', $config);
		$image_width = element('image_width', $config) ? element('image_width', $config) : '240';
        ...
        if ($cache_minute> 0) {
			...
			$cachename = 'media/script-s-' . $brd_key."-".$post_id . '-i-' . $cache_post_id . '-k-' . $cache_brd_key . '-l-k-l-t-g-w-' . $image_width . '-h-' . $image_height . '-p';
			$html = $this->CI->cache->get($cachename);
			if ($html) {
				return $html;
			}
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'media/' . $skin);
		$html = $this->CI->load->view('media/' . $skin . '/script', $view, true);
