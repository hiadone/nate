...
class Cmallitem extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
	protected $models = array('Cmall_item', 'Cmall_item_meta', 'Cmall_item_detail', 'Cmall_category', 'Cmall_category_rel', 'Cmall_channel', 'Cmall_channel_rel');
	...
	public function index()
	{
		 ...
		 */
		$this->{$this->modelname}->allow_search_field = array('cit_id', 'cit_key', 'cit_name', 'cit_datetime', 'cit_updated_datetime', 'cit_content', 'cit_mobile_content', 'cit_price','cit_direct_url'); // 검색이 가능한 필드
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$result['list'][$key]['channel'] = $this->Cmall_channel_model->get_channel(element('cit_id', $val));
		...
		$view['view']['import_url'] = admin_url($this->pagedir . '/import');
	...
	public function write($pid = 0)
	{
		...
		if ($form_validation === false OR $file_error !== '') {
			...
			$view['view']['data']['all_channel'] = $this->Cmall_channel_model->get_all_channel();
			...
			$cmall_channel = $this->input->post('cmall_channel', null, '');
			...
			if ($this->input->post($primary_key)) {
				...
				$this->Cmall_channel_rel_model->save_channel($this->input->post($primary_key), $cmall_channel);
