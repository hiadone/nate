...
class Board_write extends CB_Controller
{
	...
	public function _write_common($board, $origin = '', $reply = '')
	{
		...
		if ($use_upload === true && $form_validation && element('use_upload_file', $board)) {
			...
			$media_path ='views/media/';
			if (is_dir($media_path) === false) {
				mkdir($media_path, 0707);
				// $file = $upload_path . 'index.php';
				// $f = @fopen($file, 'w');
				// @fwrite($f, '');
				// @fclose($f);
			...
			$media_path .= element('brd_key', $board) . '/';
			if (is_dir($media_path) === false) {
				mkdir($media_path, 0707);
				// $file = $upload_path . 'index.php';
				// $f = @fopen($file, 'w');
		...
		if ($form_validation === false OR $file_error) {
			...
			$k= 0;
			$default['campaign_status']='enable';
			if ($form && is_array($form)) {
				foreach ($form as $key => $value) {
					if ( ! element('use', $value)) {
						continue;
					...
					if (element('field_type', $value) === 'text'
						...
						} elseif (element('field_type', $value) === 'number') {
                            $extra_content[$k]['input'] .= '<input type="number" id="' . element('field_name', $value) . '" name="' . element('field_name', $value) . '" class="form-control input px70" value="' . set_value(element('field_name', $value),$item) . '" ' . $required . ' />';
                        }else {
